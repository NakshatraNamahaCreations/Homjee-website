...
function PaymentCheckout() {
  ...
  // Derive the installment stage from the current booking state
  // Mirrors the same logic the backend uses (normalizeStage)
  const getCurrentInstallmentStage = () => {
    if (category === "Deep Cleaning") {
      return "final"; // DC only has first (enquiry) + final
    }
    // House Painting
    if (!firstPaid) return "first";
    if (!secondPaid) return "second";
    return "final";
  };
  ...
  const handleProceedToPay = async () => {
    ...
    const installmentStage = getCurrentInstallmentStage();

    try {
      setIsResLoading(true);

      // ── STEP 1: Call makePayment to create a Razorpay order ──
      const orderRes = await postRequest(API_ENDPOINTS.PROCEED_TO_PAY, {
        bookingId,
      ...
      const razorpayOrder = orderRes.razorpayOrder; // { keyId, orderId, amount (₹), currency, purpose }

      if (!razorpayOrder) {
        throw new Error("Razorpay order details missing from server response");
      }

      // ── STEP 2 & 3: Open Razorpay modal, then verify via makePayment (Step 2) ──
      // Using payWithRazorpay utility (same as Checkout.jsx) but with a custom
      // onVerify that calls makePayment again with the Razorpay success payload.
      await payWithRazorpay({
        bookingId,
        razorpayOrder,
        customer: bookingData?.customer,

        // Custom verify: calls makePayment endpoint (Step 2) instead of /payments/razorpay/verify
        onVerify: async (response) => {
          const verifyRes = await postRequest(API_ENDPOINTS.PROCEED_TO_PAY, {
            bookingId,
            paymentMethod: "Razorpay",
            paidAmount: amount,
            installmentStage,
            // Razorpay success fields — presence of these triggers Step 2 in makePayment
        ...
        onSuccess: async (data) => {
          console.log("Payment verified & recorded:", data);
        ...
        onFailure: (msg) => {
          if (msg && msg !== "Payment cancelled") {
            alert(msg || "Payment failed. Please contact support.");
          }
          setIsResLoading(false);
        },
      });
    } catch (error) {
      console.error("handleProceedToPay error:", error);
      ...
      <div className="container py-3" style={{ maxWidth: 560 }}>
        ...
        <div className="card shadow-sm mb-3 p-3">
          ...
          >
            ...
            {bookingData?.serviceType === "deep_cleaning" ? "24" : "24"} hrs
            before the service or if a professional isn't assigned. A fee will
