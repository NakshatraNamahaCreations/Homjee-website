...
// Case + alias insensitive equality. Use for matching cityConfigs[].city
// against the customer's selectedAddress.city.
export const sameServiceCity = (a, b) => {
  const left = resolveServiceCity(a).toLowerCase();
  const right = resolveServiceCity(b).toLowerCase();
...
//      Layout to "Bangalore" because the locality is "Bengaluru" and the
//      district is "Bengaluru Urban".
